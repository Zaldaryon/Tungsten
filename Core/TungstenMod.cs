...
namespace Tungsten
{
    public class TungstenMod : ModSystem
    {
        ...
        public override void StartServerSide(ICoreServerAPI api)
        {
            ...
            // Pre-warm all ThreadLocal instances to eliminate cold-start allocation burst
            int warmed = ThreadLocalRegistry.PreWarmAll();
            Api.Logger.Notification($"[Tungsten] Pre-warmed {warmed} ThreadLocal instances on main thread");
            ...
            api.ChatCommands.Create("tungsten")
                ...
                .BeginSubCommand("all")
                    .WithDescription("Toggle all optimizations on/off (restart required)")
                ...
                .BeginSubCommand("stats")
                    .WithDescription("Show/toggle advanced monitoring statistics")
                    .WithArgs(api.ChatCommands.Parsers.OptionalWordRange("action", onOff))
                    .HandleWith(args => tungstenCommand.Execute(args))
                .EndSubCommand()
                .BeginSubCommand("reload")
                    .WithDescription("Reload config and apply runtime-safe settings")
                    .HandleWith(args => tungstenCommand.Execute(args))
                .EndSubCommand()
                .BeginSubCommand("diag")
                    .WithDescription("Diagnostic modules control")
                ...
                .BeginSubCommand("benchmarkharness")
                    .WithDescription("Control A/B benchmark harness")
                    .WithArgs(api.ChatCommands.Parsers.OptionalWordRange("action", onOff))
                    .HandleWith(args => tungstenCommand.Execute(args))
                .EndSubCommand()
                .BeginSubCommand("frameprofiler")
                    .WithDescription("Control vanilla frame profiler integration")
                ...
                .BeginSubCommand("health")
                    .WithDescription("Show multi-mod patch conflicts and event handler counts")
                    .HandleWith(args => tungstenCommand.Execute(args))
                .EndSubCommand()
                .BeginSubCommand("manifest")
                    .WithDescription("Dump IL signature hashes to server log")
                    .HandleWith(args => tungstenCommand.Execute(args))
                .EndSubCommand()
                .BeginSubCommand("propickreadingoptimization")
                    .WithArgs(api.ChatCommands.Parsers.WordRange("action", onOff))
                    .HandleWith(args => tungstenCommand.Execute(args))
                .EndSubCommand()
                .BeginSubCommand("genterrazeroallocoptimization")
                    .WithArgs(api.ChatCommands.Parsers.WordRange("action", onOff))
                    .HandleWith(args => tungstenCommand.Execute(args))
                .EndSubCommand()
                .BeginSubCommand("genterrabitarrayoptimization")
                    .WithArgs(api.ChatCommands.Parsers.WordRange("action", onOff))
                    .HandleWith(args => tungstenCommand.Execute(args))
                .EndSubCommand()
                .HandleWith(args => tungstenCommand.Execute(args));
